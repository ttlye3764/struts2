		...
		<div id="myCarousel" class="carousel slide" data-ride="carousel">
			...
			<c:forEach items="${imageBoardInfo.items }" var="fileItemInfo" varStatus="stat">
				...
				<c:if test="${stat.last }">
					...
					<img src="/files/${fileItemInfo.img_save_name}" onclick="javascript:location.href='/user/imageFileDown.do?img_seq=${fileItemInfo.img_seq }'">
				</div>
			</c:forEach>
...
<script type ="text/javascript">
	...
	$(function(){
		...
		// 글쓰기
		$('#btn1').click(function(){
			$(location).attr('href', '/user/imageboardForm.do');
		});
		...
		$('#btn2').click(function(){
			...
			if(eval('${!empty LOGIN_SOLDIERINFO}')){ // 로그인 되어져야하고
				if('${LOGIN_SOLDIERINFO.sol_num}'== '${imageBoardInfo.img_writer}'){ // 해당게시글 작성자여야 하고
					$(location).attr('href', '/user/imageboardDelete.do?img_no=${imageBoardInfo.img_no}');		
				}else{
		...
		$('#btn4').click(function(){
			...
			$(location).attr('href', '/user/imageboardList.do');
		});
		...
		$('form[name=imageboardView]').submit(function(){
			...
			if(eval('${!empty LOGIN_SOLDIERINFO}')){
				if('${LOGIN_SOLDIERINFO.sol_num}'== '${imageBoardInfo.img_writer}'){
					...
					$(this).attr('action', '/user/imageboardUpdate.do');
